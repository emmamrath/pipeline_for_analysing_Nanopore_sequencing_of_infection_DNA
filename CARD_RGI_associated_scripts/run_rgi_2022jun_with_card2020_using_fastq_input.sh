...
currdir=$outdir
output_prefix="${currdir}"/output_rgi_2022jun_with_card2020
...
echo ''
echo 'awk' '{if ((NR % 4) == 1) {sub(/@/,">"); print $0}; if ((NR % 4) == 2) {print $0}}' $in_fastq '>' $tmp_fasta
awk '{if ((NR % 4) == 1) {sub(/@/,">"); print $0}; if ((NR % 4) == 2) {print $0}}' $in_fastq > $tmp_fasta
echo ''
